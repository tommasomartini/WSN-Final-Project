...
void DataCollector::report() {
	...
	if (measures_register_.size() > 0) {
		...
		ofstream my_file("./../data_simulation_folder/c1_simul.txt", ios::app);
		if (my_file.is_open()) {
			my_file << MyToolbox::C1_ << "," << avg_msr_life_time * 1. / pow(10, 9) << "," << avg_not_crossed * 1. / num_msrs << endl;
			my_file.close();
		}
	}
...
//	cout << "+ SOLITON CHECK" << endl;
	...
	cout << "+ CACHE FRESHNESS" << endl;
	ofstream cache_freshness_file("./../data_simulation_folder/cache_freshness.txt", ios::app);
	if (cache_freshness_file.is_open()) {
		double overall_avg_freshness = 0;	// sum of #update_msr / #followed_sensors
		double overall_avg_freshness_w = 0;
		double overall_avg_diff = 0;	// sum of diff / #msr generated by all the sensor I am following
		double overall_avg_diff_w = 0;
		int num_updted_cache = 0;	// how many caches are updated with every sensor they follow
		int num_updted_cache_w = 0;
		map<unsigned int, StorageNode> my_caches = MyToolbox::storage_nodes_map_;	// copy the cache map
		for (map<unsigned int, StorageNode>::iterator cache_it = my_caches.begin(); cache_it != my_caches.end(); cache_it++) {	// for each cache
			map<unsigned int, unsigned int> most_recent_msrs = cache_it->second.get_most_recent_measures();	// get its most recent measures
			map<unsigned int, unsigned int> most_recent_msrs_w = cache_it->second.last_msr_without_backward_;	// if I didn't have the backward
			for (map<unsigned int, unsigned int>::iterator msr_it = most_recent_msrs.begin(); msr_it != most_recent_msrs.end(); msr_it++) {
				if (most_recent_msrs_w.find(msr_it->first) == most_recent_msrs_w.end()) {
					most_recent_msrs_w.insert(pair<unsigned int, unsigned int>(msr_it->first, msr_it->second));
				}
			}
			int num_msr = most_recent_msrs.size();
			int fresh_diff = 0;
			int fresh_diff_w = 0;
			int fresh_denominator = 0;
			int num_up_msr = 0;
			int num_up_msr_w = 0;

			// With backward
			for (map<unsigned int, unsigned int>::iterator msr_it = most_recent_msrs.begin(); msr_it != most_recent_msrs.end(); msr_it++) {	// for each most recent measure
				unsigned int sns_id = msr_it->first;	// sensor id who generated this measure
				fresh_denominator += measure_data_all_register_.find(sns_id)->second.size();	// sum the amount of measure generated by this sensor
				unsigned int my_msr_id = msr_it->second;	// the most updated measure according to the cache
				unsigned int next_msr_id = my_msr_id + 1;	// the next measure there would be after the one owned by the cache
				MeasureKey next_key(sns_id, next_msr_id);
				if (measure_data_register_.find(next_key) != measure_data_register_.end()) {	// there is a more recent measure
					// do not increment cache's number of update measure. This measure is not the most recent
			...
			if (num_msr == num_up_msr) {
				num_updted_cache++;
			}
			double avg_freshness = num_up_msr * 1. / num_msr;
			overall_avg_freshness += avg_freshness;
			if (fresh_denominator > 0) {
				double normalized_diff = fresh_diff * 1. / fresh_denominator;
				overall_avg_diff += normalized_diff;
			}
			cache_freshness_file << "Cache" << cache_it->first << "," << num_msr << "," << num_up_msr << "," << fresh_diff << "," << avg_freshness << endl;

			// Without backward
			for (map<unsigned int, unsigned int>::iterator msr_it = most_recent_msrs_w.begin(); msr_it != most_recent_msrs_w.end(); msr_it++) {	// for each most recent measure
				unsigned int sns_id = msr_it->first;	// sensor id who generated this measure
				unsigned int my_msr_id = msr_it->second;	// the most updated measure according to the cache
				unsigned int next_msr_id = my_msr_id + 1;	// the next measure there would be after the one owned by the cache
				MeasureKey next_key(sns_id, next_msr_id);
				if (measure_data_register_.find(next_key) != measure_data_register_.end()) {	// there is a more recent measure
					// do not increment cache's number of update measure. This measure is not the most recent
					fresh_diff_w++;	// difference is at least one
					next_key.measure_id_++;		// the next measure
					while (measure_data_register_.find(next_key) != measure_data_register_.end()) {
						fresh_diff_w++;
						next_key.measure_id_++;
			...
			if (num_msr == num_up_msr_w) {
				num_updted_cache_w++;
			}
			double avg_freshness_w = num_up_msr_w * 1. / num_msr;
			overall_avg_freshness_w += avg_freshness_w;
			if (fresh_denominator > 0) {
				double normalized_diff_w = fresh_diff_w * 1. / fresh_denominator;
				overall_avg_diff_w += normalized_diff_w;
			}
		}
		overall_avg_freshness = overall_avg_freshness / MyToolbox::num_storage_nodes_;
		overall_avg_freshness_w = overall_avg_freshness_w / MyToolbox::num_storage_nodes_;
		double overall_avg_diff_all = overall_avg_diff / MyToolbox::num_storage_nodes_;
		double overall_avg_diff_part = overall_avg_diff / (MyToolbox::num_storage_nodes_ - num_updted_cache);
		double overall_avg_diff_all_w = overall_avg_diff_w / MyToolbox::num_storage_nodes_;
		double overall_avg_diff_part_w = overall_avg_diff_w / (MyToolbox::num_storage_nodes_ - num_updted_cache_w);
		cout << " - Avg freshness: " << overall_avg_freshness << endl;
		cout << " - Avg normalized difference: " << overall_avg_diff_all << endl;
		cout << " - Avg normalized difference (partial): " << overall_avg_diff_part << endl;
		cout << " - Avg freshness (no BW): " << overall_avg_freshness_w << endl;
		cout << " - Avg normalized difference (no BW): " << overall_avg_diff_all_w << endl;
		cout << " - Avg normalized difference (partial, no BW): " << overall_avg_diff_part_w << endl;
		cache_freshness_file << "Avg freshness: " << overall_avg_freshness << endl;
		cache_freshness_file << "Avg normalized difference: " << overall_avg_diff_all << endl;
		cache_freshness_file << "Avg normalized difference (partial): " << overall_avg_diff_part << endl;
		cache_freshness_file << "Avg freshness (no BW): " << overall_avg_freshness_w << endl;
		cache_freshness_file << "Avg normalized difference (no BW): " << overall_avg_diff_all_w << endl;
		cache_freshness_file << "Avg normalized difference (partial, no BW): " << overall_avg_diff_part_w << endl;
...
//		}
		...
		ofstream myfile("./../data_simulation_folder/user_replacement.txt", ios::app);
		if (myfile.is_open()) {
			map<unsigned int, unsigned int> replacements_copy = user_replacement_register_;
			vector<unsigned int> to_remove_entries;
			bool stop = false;
...
void DataCollector::clear_registers() {
	num_stored_measures_per_cache_.clear();
	measures_register_.clear();
	measure_data_register_.clear();
	measure_data_all_register_.clear();
	blacklist_register_.clear();
	user_register_.clear();
	user_replacement_register_.clear();
	cache_freshness_register_.clear();
}
